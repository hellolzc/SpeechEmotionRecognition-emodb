...
class DLDataSets():
    ...
    """
    def __init__(self, label_se, file_path, class_num):
        self.data_file_path = file_path  # 数据文件的位置
        self.label_se = label_se.copy()  # 标签信息
        self.data_length = None  # 一个utterace的长度
        self.class_num = class_num
        print('HDF5 File: %s' % file_path)
    ...
    def get_data_scaled(self, seed, ith, normlize=True):
        ...
        with h5py.File(self.data_file_path, "r") as feat_clps:
            if normlize:
                sc = _fit_X_scaler(data_shape, feat_clps, train_index)
            else:
                sc = None
            X_train = _get_X_scaled(data_shape, feat_clps, train_index, scaler=sc)
            X_test = _get_X_scaled(data_shape, feat_clps, test_index, scaler=sc)
